...
def EvaluateCompletion(data,method,useRelation,execTimes,logger,unobservedRates = None,alpha=None,ranks=None):
    EvaluateCompletionMain(data,method,useRelation,execTimes,logger,unobservedRates,alpha,ranks)
    try:
        EvaluateCompletionMain(data,method,useRelation,execTimes,logger,unobservedRates,alpha,ranks)
    except Exception,e:
        print e
...
def EvaluateCompletionMain(data,method,useRelation,execTimes,logger,unobservedRates = None,alpha=None,ranks=None):
    global log 
    log = logger
    ...
    maskAxis = 0
    elems = prod(X.shape) 
    targetelems = prod(X.shape) / X.shape[maskAxis]
